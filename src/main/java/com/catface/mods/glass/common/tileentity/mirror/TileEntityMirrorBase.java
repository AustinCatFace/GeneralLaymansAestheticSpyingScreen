...
public class TileEntityMirrorBase extends TileEntity implements ITickable {
    ...
    public void propagate() //do I need to send active state, channel, online/offline, block change/init propagation?
    {
        ...
        for(EnumFacing facing : propagationFaces)
        {
            ...
            if(te instanceof TileEntityMirrorBase)
            {
                ((TileEntityMirrorBase)te).bePropagatedTo(this, channel, active);
            }
        }
        if(!active)
        {
    ...
    public void bePropagatedTo(TileEntityMirrorBase base, String newChannel, boolean activate)
    {
        boolean flag = false;
        ...
        if(!activate && active && channel.equalsIgnoreCase(newChannel)) //turn off
        {
            ...
            {
                propagateTime = TileEntityMirrorBase.PROPAGATE_TIME + 1;
                IBlockState state = getWorld().getBlockState(getPos());
                getWorld().notifyBlockUpdate(getPos(), state, state, 3);
            }
        ...
        if(flag)
        {
            fadeoutTime = TileEntityMirrorBase.FADEOUT_TIME;
            propagateTime = TileEntityMirrorBase.PROPAGATE_TIME;
            IBlockState state = getWorld().getBlockState(getPos());
            getWorld().notifyBlockUpdate(getPos(), state, state, 3);
        }
    }

    public void checkFacesToTurnOn(TileEntityMirrorBase origin)
    {
        if(origin != this)
        {
            activeFaces.clear();
            ...
            if(activeFaces.size() > 1)
            {
                for(int i = activeFaces.size() - 1; i >= 0; i--)
                {
                    ...
                    if(te instanceof TileEntityMirrorBase && ((TileEntityMirrorBase)te).active && ((TileEntityMirrorBase)te).channel.equalsIgnoreCase(channel) && ((TileEntityMirrorBase)te).distance < distance)
                    {
            ...
            for(EnumFacing facing : activeFaces)
            {
                ...
                TileEntity te = getWorld().getTileEntity(facePos);
                if(te instanceof TileEntityMirrorBase) //inner corner
                {
                        ...
                        if(origin.getPos().getY() != getPos().getY())
                        {
                            ...
                            for(EnumFacing newFacing : PROPAGATION_FACES.get(facing))
                            {
                                if(newFacing.getAxis() != EnumFacing.Axis.Y)
                                {
                                    ...
                                    if(te1 instanceof TileEntityMirrorBase && ((TileEntityMirrorBase)te1).activeFaces.contains(facing))
                                    {
                                        origin = ((TileEntityMirrorBase)te1);
                                    }
                                }
                            }
                        }
