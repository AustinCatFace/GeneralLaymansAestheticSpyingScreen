...
//        drawPlanes(te, 1F, 1F, 0F, 1F, 0.502D);
    ...
    public void drawScene(TileEntityGlassBase te, float partialTick)
    {
        if(WorldPortalRenderer.renderLevel == 0 && !CFGlass.eventHandlerClient.drawnChannels.contains(te.channel))
        {
            ...
            TerminalPlacement placement = CFGlass.eventHandlerClient.getTerminalPlacement(te.channel);
            ...
            if(placement != null)
            {
                ...
                for(TileEntityGlassBase base : placement.activeBlocks)
                {
                    if(base.active && base.lastDraw > 0)
                    {
                        ...
                        TileEntityGlassRenderer.drawPlanes(base, 1.0f, 1.0f, 1.0f, 1F, 0.501D, partialTick);
                ...
                GlStateManager.enableCull();
                OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240F, 240F);
            } else if (te.channel.equals("default") && te instanceof TileEntityGlassMaster){
                placement = new TerminalPlacement(te.getWorld(), (TileEntityGlassMaster) te,te, CFGlass.eventHandlerClient.getActiveGlass(te.channel));
                placement.renderCaller = te;
                Minecraft mc = Minecraft.getMinecraft();
                Entity entity = mc.getRenderViewEntity();
